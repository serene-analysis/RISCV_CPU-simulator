...
signed main(){
    ...
    dmem.DMEM_next = dmem.DMEM = conv.IMEM;
    uint_32 PC = 0, PC_next = 0;
    int timestamp = 0;
    bool onl = false;
    #ifdef ONLINE_JUDGE
    onl = true;
    ...
    while(true && (++timestamp < 20 || onl)){
        ...
        PC_next = MUX(ADD(PC, 4), MUX(cleared, alu_result, info.alu_src_a), info.is_jump | (info.is_branch & condition_met));
